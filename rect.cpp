...
//isoverlapping method for two rectangles
bool Rect::isOverlapping(const Rect &r) const {
    ...
    if (r.getBottomY() < getTopY() || r.getTopY() > getBottomY()) {
        return false;
    }
    else if (r.getRightX() < getLeftX() || r.getLeftX() > getRightX()) {
        return false;
    }
...
void Rect::draw() const {
    ...
    glBegin(GL_QUADS);
    glVertex2i(center.x - (size.width / 2.0), center.y - (size.height / 2.0));
    glVertex2i(center.x - (size.width / 2.0), center.y + (size.height / 2.0));
    glVertex2i(center.x + (size.width / 2.0), center.y + (size.height / 2.0));
    glVertex2i(center.x + (size.width / 2.0), center.y - (size.height / 2.0));
    glEnd();
